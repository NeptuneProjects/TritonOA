...
class SoundSpeedFunctions:
    ...
    def sos_DelGrosso(Z, S, T):
        ...
        def _at_each_depth(P, S, T):
            ...
            C000 = 1402.392
            CT = np.array([0.0, 5.012285, -0.551184e-1, 0.221649e-3])
            CS = np.array([0.0, 1.329530, 0.1288598e-3])
            CP = np.array([0.0, 0.1560592, 0.2449993e-4, -0.8833959e-8])
            C_ST = -0.1275936e-1
            C_TP = 0.6353509e-2
            C_T2P2 = 0.2656174e-7
            C_TP2 = -0.1593895e-5
    ...
    def sos_UNESCO(Z, S, T):
        ...
        def _at_each_depth(P, S, T):
            ...
            AMAT = np.array(
                [
                    [1.389, -1.262e-2, 7.166e-5, 2.008e-6, -3.21e-8, 0.0],
                    [9.4742e-5, -1.2583e-5, -6.4928e-8, 1.0515e-8, -2.0142e-10, 0.0],
                    [-3.9064e-7, 9.1061e-9, -1.6009e-10, 7.994e-12, 0.0, 0.0],
                    [1.100e-10, 6.651e-12, -3.391e-13, 0.0, 0.0, 0.0],
                ]
            )
            BMAT = np.array(
                [
                    [-1.922e-2, -4.42e-5, 0.0, 0.0, 0.0, 0.0],
                    [7.3637e-5, 1.7950e-7, 0.0, 0.0, 0.0, 0.0],
                    np.zeros(len(nT)),
                    np.zeros(len(nT)),
                ]
            )
            ...
            DMAT = np.array([1.727e-3, -7.9836e-6, 0.0, 0.0])
    ...
    def gravity(latitude):
        ...
        g = (
            G1
            * (1 + G2 * np.sin(latitude) ** 2)
            / np.sqrt(1 - G3 * np.sin(latitude) ** 2)
        )
